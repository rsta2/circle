...
#if RASPPI >= 5
	if (m_nBus == PCIE_BUS_ONBOARD)
	{
		// another window for RP1 SRAM
		write32 (conf + PCI_BASE_ADDRESS_2, lower_32_bits (MEM_PCIE_RANGE_PCIE_START2));
		write32 (conf + PCI_BASE_ADDRESS_3, upper_32_bits (MEM_PCIE_RANGE_PCIE_START2));
	}
#endif
