 ...
 * \param channel: virtual channel id
 * \param type: payload data type
 * \param flags: flags controlling this message transmission
 * \param tx_len: length of tx_buf
 * \param tx_buf: data to be written
 * \param rx_len: length of rx_buf
 * \param rx_buf: data to be read, or NULL
 */
struct mipi_dsi_msg {
	u8 channel;
	u8 type;
 ...
 * \param header: the four bytes that make up the header (Data ID, Word Count or
 *     Packet Data, and ECC)
 * \param payload_length: number of bytes in the payload
 * \param payload: a pointer to a buffer containing the payload, if any
 */
 ...
 * \param attach: attach DSI device to DSI host
 * \param detach: detach DSI device from DSI host
 * \param transfer: transmit a DSI packet
 *
 ...
 * \param dev: driver model device node for this DSI host
 * \param ops: DSI host operations
 * \param list: list management
 */
struct mipi_dsi_host {
	// struct device *dev;
 ...
 * \param type: DSI peripheral chip type
 * \param channel: DSI virtual channel assigned to peripheral
 * \param node: pointer to OF device node or NULL
 *
 ...
 * \param host: DSI host for this peripheral
 * \param dev: driver model device node for this peripheral
 * \param attached: the DSI device has been successfully attached
 * \param name: DSI peripheral chip type
 * \param channel: virtual channel assigned to the peripheral
 * \param format: pixel format for video mode
 * \param lanes: number of active data lanes
 * \param mode_flags: DSI operation mode related flags
 * \param hs_rate: maximum lane frequency for high speed mode in hertz, this should
 * be set to the real limits of the hardware, zero is only accepted for
 * legacy drivers
 * \param lp_rate: maximum lane frequency for low power mode in hertz, this should
 * be set to the real limits of the hardware, zero is only accepted for
 * legacy drivers
 * \param dsc: panel/bridge DSC pps payload to be sent
 */
struct mipi_dsi_device {
...
struct mipi_dsi_multi_context {
	/**
	 * \param dsi: Pointer to the MIPI DSI device
	 */
	struct mipi_dsi_device *dsi;

	/**
	 * \param accum_err: Storage for the accumulated error over the multiple calls
	 *
 ...
 *                                specification
 * \param fmt: MIPI DSI pixel format
 *
 ...
 * \param MIPI_DSI_DCS_TEAR_MODE_VBLANK: the TE output line consists of V-Blanking
 *    information only
 * \param MIPI_DSI_DCS_TEAR_MODE_VHBLANK : the TE output line consists of both
 ...
 * \param seq: buffer containing the payload
 */
 ...
 *
 * \param ctx: Context for multiple DSI transactions
 * \param seq: buffer containing the payload
 */
 ...
 * \param dsi: DSI peripheral device
 * \param cmd: Command
 * \param seq: buffer containing data to be transmitted
 */
 ...
 * \param driver: device driver model driver
 * \param probe: callback for device binding
 * \param remove: callback for device unbinding
 * \param shutdown: called at shutdown time to quiesce the device
 */
